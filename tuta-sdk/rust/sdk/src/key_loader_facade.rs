...
impl KeyLoaderFacade {
	...
	pub async fn load_sym_group_key(
		...
		if group_key.version == version {
			...
			// FIXME: refresh if group_key.version < version
	...
	async fn find_former_group_key(
		&self,
		group: &Group,
		// FIXME why do we take it by ref if we are cloning it anyway
	...
	async fn load_sym_user_group_key(
		...
		// FIXME: check for the version and refresh cache if needed
